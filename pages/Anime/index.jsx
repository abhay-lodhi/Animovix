...
const Anime = () => {
  ...
  window.alert("YOU HAVE REACHED THE MAXIMUM INPUT LIMIT !!")
  }

  const removeBar = ()=>{
    if(bars.length>1)setBars(s=>{
      return  s.slice(0,-1)
    })
    else{
  ...
  return (
    ...
    <div className={styles.contain}>
      ...
      {/* <Searchanime/> */}
      {bars.map((val, i)=>{
      return <Container  style={{ width: "900", borderRadius:"10px", margin:"5.5rem 1.5rem" }} className="container">
  ...
  <Button bordered className={styles.btn} color="gradient" auto onPress={addBar}>
        ...
        <Container>
        <Button bordered disabled={loading} className={styles.btn} color="primary" auto onPress={getRecommendations}>{loading?<Loading type="points" color="currentColor" size="sm" />:`Show Recommendations`}
    ...
    {ask &&<Container className={styles.recommendcon} fluid display='flex'  justify='space-evenly'>
      ...
      <Pagination shadow bordered css={{display:"flex",m:"2rem"}}  color="secondary" total={Math.ceil(100/itemcount)} page={curpage} initialPage={1} onChange={page=>setCurpage(page)}/>
      ...
      {console.log(results)}
        ...
        <Pagination bordered shadow css={{display:"flex",m:"2rem"}} color="secondary" total={Math.ceil(100/itemcount)}  page={curpage} initialPage={1} onChange={page=>setCurpage(page)}/>
      </Container>:<Loading color="secondary" type="gradient"
      
      loadingCss={{ $$loadingSize: "100px", $$loadingBorder: "10px", margin:'10vw auto' }}
    />}
        </Container>}
    </div>
  )
