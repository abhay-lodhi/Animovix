...
const Profilecard = ({details, badge}) => {
  ...
  const [selected, setSelected] = useState(new Set([badge==="favorite"?"Status":badge]));
  ...
  const updateList = async (key) => {   
    ...
    setIsChanging(true);
    await updateUserLists(detA, mapVal[key], mapVal[selectedValue]===undefined?null:mapVal[selectedValue], (type2==="Anime"));
    setIsChanging(false);
   };
  ...
  const reRoute=()=>{
    router.push(type2==="Manga"?`/MangaSeries/${id}`:`/AnimeSeries/${id}`);
  }
  ...
  return (
    ...
    <div className={styles.temporary_text} onClick={()=>reRoute()}>
      <Image src={imageUrl} fill alt='Anime/Manga Image' sizes="(max-width: 768px) 50vw, (max-width: 1200px) 33vw, 20vw"/>
    </div>
<div className={styles.card_content}>
    <p className={styles.card_title}>{title.substr(0, 35)+(title.length>35?"...":"")} </p>
        <span className={styles.card_subtitle}>
          <ul>
            <li>{type2}</li> 
            <li>{type2==="Manga"?`chapters : ${details.chapters}`:`episodes : ${details.episodes}`} </li>
          </ul> 
          </span>
        <div className={styles.card_description}><p className={styles.synopsis}>{synopsis.substr(0,70)+(synopsis.length>70?"...":"")}</p> <br></br>
       <div className={styles.under_synopsis}>
        {badge==="favorite" && (favourite ?
         <Tooltip content={"Remove from Favorites"} rounded color="secondary">
          <PiHeartStraightFill size={30} color='#c9000091' onClick={()=>updateFavourite()}/>
        </Tooltip>
         :<Tooltip content={"Add to Favorites"} rounded color="secondary">
           <PiHeartStraightBold size={30} color='#c9000091' onClick={()=>updateFavourite()}/>
         </Tooltip>)}
